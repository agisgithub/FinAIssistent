...
O monitor opcional de novos gastos é a única exceção fechada à confirmação individual. Ele ignora o primeiro snapshot completo, transferências, splits, saldo inicial, entradas, contas encerradas/fora do orçamento e lançamentos já categorizados. Escrita automática exige uma única recomendação de alta confiança, escore mínimo `0.95`, sem conflito e fonte `rule` ou `confirmed`. Imediatamente antes da reserva, a aplicação relê fingerprint, política e destino; depois reutiliza o mesmo backup, patch de categoria, sincronização e releitura. A operação recebe origem `companion_high_confidence`, não alimenta exemplos confirmados e um resultado incerto nunca é repetido. Qualquer outro caso gera pergunta limitada e deduplicada no Telegram; uma categorização externa invalida pergunta/proposta pendente.
