...
`interpret(text,{today})` retorna `{intent,metadata}`. Uma consulta contém exatamente `{kind,period:{start,end},page}`. `kind` é um dos valores `summary`, `spending`, `budget`, `uncategorized`, `leaks` ou `accounts`; o período é inclusivo, válido, não futuro e limitado a 24 meses; a página vai de 1 a 100000. Para escrita, pedido fora do escopo ou interpretação insuficiente, o schema permite a recusa explícita `{kind:'unsupported'}` sem campos adicionais; o chamador explica o limite sem consultar o Actual. Campos extras, IDs, valores monetários, métodos SDK, ferramentas e ações são recusados. A validação local continua obrigatória mesmo com JSON Schema no modelo.
