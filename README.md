...
As sugestões de `/sugerir` seguem regras explícitas locais, exemplos confirmados ativos e histórico de até 12 meses, nessa ordem. O escore mede evidência, não probabilidade. Confiança alta exige regra sem conflito ou ao menos cinco exemplos únicos em acordo sem conflito; 5/5 pode ser alta, 9/10 fica conservadoramente média. O monitor 3B é desligado por padrão, faz um primeiro baseline silencioso e depois verifica, a cada 15 minutos, apenas novas despesas simples sem categoria em contas abertas e orçamentárias. Com `transactionMonitorEnabled:true`, casos incertos geram no máximo três perguntas por varredura. Com `autoCategorizeHighConfidence:true`, `dryRun:false` e chave de backup, até 20 destinos únicos de alta confiança por varredura podem ser aplicados pelo mesmo fluxo de backup, fingerprint, patch, sincronização e releitura; resultados incertos nunca são repetidos. Memórias aparecem somente como contexto na pergunta e jamais aumentam escore ou autorizam automação. Separadamente, a conversa pode consultar categorias reais e preparar uma proposta de até dez alterações ou criar categoria em grupo existente; toda execução exige confirmação. Lotes podem concluir parcialmente; itens incertos não são repetidos. Exemplo de regra configurada, com IDs reais escolhidos pelo operador:
