...
import { label, displayDate, displayTime } from './render.mjs';
...
function upcomingText(upcoming, reportDate, size, compact = false) {
  ...
  if (!items.length) return upcoming.registeredCount === 0 ? 'Próximos vencimentos — calendário local: nenhuma recorrência cadastrada.' : `Próximos vencimentos — calendário local: nenhum vencimento informado de ${displayDate(reportDate)} a ${displayDate(through)}.`;
  return `Próximos vencimentos — calendário local, ${displayDate(reportDate)} a ${displayDate(through)}:\n${items.slice(0, size).map(item => `${displayDate(item.dueDate)} (${item.dateKind === 'confirmed' ? 'confirmado' : 'estimado'}): ${label(item.name, compact ? 32 : 90)}, ${money(item.amountCents)}.`).join('\n')}${items.length > size ? `\nExibidos ${size} de ${items.length}; /proximos_vencimentos para a lista.` : ''}`;
}
...
export function buildDailyReport(snapshot, { identity, reportDate, today, scope = DEFAULT_SCOPE, dataState = 'fresh', detail = 'summary', thresholds, upcoming, trackedAlerts = [] } = {}) {
  ...
  const lines = detail === 'summary' ? [
    ...
    `RELATÓRIO DIÁRIO · ${displayDate(reportDate)}\n${reportDate === today ? 'Dia e mês em andamento.' : 'Dia encerrado.'}`,
    `Saldos até ${displayDate(reportDate)}:`,
    ...
    `Mês ${displayDate(monthPeriod.start)} a ${displayDate(reportDate)}: despesas líquidas ${formatMoney(month.totals.netExpenses)}.`,
    ...
    `Incomuns (${displayDate(anomalies.window.start)} a ${displayDate(reportDate)}): ${anomalies.findings.length}; base insuficiente: ${anomalies.insufficient} (mín. 8 anteriores).`,
    shortList(anomalies.findings, finding => {
      const row = whole.includedTransactions.find(item => item.id === finding.targetId);
      return `${displayDate(finding.date)} ${brief(whole.payees.get(row.payeeId)?.name)} / ${brief(whole.accounts.find(account => account.id === row.accountId)?.name)}: ${formatMoney(finding.amount)}; mediana ${formatMoney(finding.medianCents)}.`;
    ...
    `Fonte: Actual · sincronizado em ${displayTime(snapshot.syncedAt, snapshot.timezone)}.`,
    `Base completa ${displayDate(period.start)} a ${displayDate(period.end)}; BRL. Escopo: ${whole.selectedAccounts.length} contas incluídas; ${whole.metadata.excludedAccountIds.length} excluídas (${scope.includeOffBudget ? 'inclui' : 'exclui'} fora do orçamento, ${scope.includeClosed ? 'inclui' : 'exclui'} encerradas).`
  ].filter(Boolean) : [
    ...
    `RELATÓRIO DIÁRIO · ${displayDate(reportDate)}\n${reportDate === today ? 'Dia e mês em andamento.' : 'Dia encerrado; alterações retroativas podem mudar os números.'}`,
    `Fatos — saldos das contas até ${displayDate(reportDate)}:`,
    ...
    `Despesas no mês, ${displayDate(monthPeriod.start)} a ${displayDate(reportDate)}: líquido ${formatMoney(month.totals.netExpenses)} (bruto ${formatMoney(month.totals.grossExpenses)}; estornos ${formatMoney(month.totals.refunds)}).`,
    ...
    `Padrões — gastos incomuns de ${displayDate(anomalies.window.start)} a ${displayDate(reportDate)}: ${anomalies.findings.length}; base insuficiente: ${anomalies.insufficient} (mínimo de ${anomalies.minimumSample} observações anteriores).`,
    ...
    detail === 'detailed' && month.uncategorized.length ? list(month.uncategorized, row => `${displayDate(row.date)} — ID ${label(row.id, 128)}: ${formatMoney(row.amount)}.`, `/sem_categoria ${monthPeriod.start} ${reportDate}`) : '',
    ...
    `Fonte: Actual · sincronizado em ${displayTime(snapshot.syncedAt, snapshot.timezone)}.`,
    `Cobertura completa: ${displayDate(period.start)} a ${displayDate(period.end)}; BRL. Escopo: ${scope.includeOffBudget ? 'inclui' : 'exclui'} fora do orçamento; ${scope.includeClosed ? 'inclui' : 'exclui'} encerradas. ${whole.selectedAccounts.length} contas incluídas; ${whole.metadata.excludedAccountIds.length} excluídas.`,
    'Fatos, cálculos e padrões separados; hipóteses causais não confirmadas.'
  ].filter(Boolean);
