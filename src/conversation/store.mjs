...
export class ConversationStore {
  ...
  digest() { return createHash('sha256').update(JSON.stringify({ generation: this.session().generation, rows: this.history() })).digest('hex'); }
  replay(job) {
  ...
  begin(request, job) {
    ...
    const id = randomUUID();
    this.db.prepare(`INSERT INTO conversation_turns(id,source_job_id,household_id,budget_id,user_id,chat_id,generation,state,created_at,user_text) VALUES(?,?,?,?,?,?,?,'running',?,?)`).run(id, job?.id ?? null, ...this.keys, this.session().generation, this.store.now(), limited(safeHistoryText(request.type === 'message' ? request.text : '[botão de confirmação acionado pelo responsável]'), 4096));
    return id;
  }
  finish(turnId, response, { selection = null, provider = null, model = null } = {}) {
    ...
    this.db.prepare(`UPDATE conversation_turns SET state='completed',assistant_text=?,selection_json=?,response_json=?,provider=?,model=? WHERE id=? AND ${KEYS}`).run(limited(safeHistoryText(response.text), maxPart), selectionJson, JSON.stringify(clean), provider, model, turnId, ...this.keys);
  ...
  consent(mode, question = null) {
    const nonce = randomBytes(18).toString('base64url'), now = this.store.now();
    this.db.prepare(`INSERT INTO conversation_consents(nonce,household_id,budget_id,user_id,chat_id,generation,context_hash,mode,question,created_at,expires_at) VALUES(?,?,?,?,?,?,?,?,?,?,?)`).run(nonce, ...this.keys, this.session().generation, this.digest(), mode, question == null ? null : safeHistoryText(question), now, now + 15 * 60000);
    return nonce;
  }
  consumeConsent(nonce, identity, cancel = false) {
    this.assert(identity);
    return this.store.transaction(() => {
      ...
      if (!cancel && (row.generation !== this.session().generation || row.context_hash !== this.digest())) throw new AppError('PROPOSAL_POLICY_CHANGED');
