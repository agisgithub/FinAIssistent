...
test('migrations 004-007 upgrade a database containing only 001-003 without changing report preferences',t=>{
  ...
  const upgraded=new StateStore(file,identityFromConfig(config));try{assert.deepEqual(upgraded.db.prepare('SELECT name FROM schema_migrations ORDER BY name').all().map(row=>row.name),['001_core.sql','002_operations.sql','003_reports.sql','004_bills.sql','005_assistant_actions.sql','006_conversations.sql','007_outbox_media.sql']);assert.ok(upgraded.db.prepare("SELECT name FROM sqlite_master WHERE name='bill_proposals'").get());assert.deepEqual(upgraded.getPreference('finance_scope'),{includeClosed:true,includeOffBudget:false});assert.equal(upgraded.db.prepare("SELECT value FROM metadata WHERE key='telegram_next_send_at'").get().value,'123456');}finally{upgraded.close();}
});
