...
test('migration 004 upgrades a database containing only 001-003 without changing report preferences',t=>{
  ...
  const upgraded=new StateStore(file,identityFromConfig(config));try{assert.equal(upgraded.db.prepare('SELECT COUNT(*) n FROM schema_migrations').get().n,4);assert.ok(upgraded.db.prepare("SELECT name FROM sqlite_master WHERE name='bill_proposals'").get());assert.deepEqual(upgraded.getPreference('finance_scope'),{includeClosed:true,includeOffBudget:false});assert.equal(upgraded.db.prepare("SELECT value FROM metadata WHERE key='telegram_next_send_at'").get().value,'123456');}finally{upgraded.close();}
});
