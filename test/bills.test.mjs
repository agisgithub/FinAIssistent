...
test('migrations 004-010 upgrade a database containing only 001-003 without changing report preferences',t=>{
  ...
  db.prepare('INSERT INTO households VALUES(?,?,?)').run(config.householdId,config.timezone,config.currency);db.prepare("INSERT INTO operations(id,household_id,budget_id,kind,state,created_at,updated_at) VALUES('old-op',?,?,'category','applied',1,1)").run(config.householdId,config.actual.budgetId);db.prepare('INSERT INTO preferences VALUES(?,?,?)').run(config.householdId,'finance_scope',JSON.stringify({includeClosed:true,includeOffBudget:false}));db.prepare('INSERT INTO metadata VALUES(?,?)').run('telegram_next_send_at','123456');db.close();
  const upgraded=new StateStore(file,identityFromConfig(config));try{assert.deepEqual(upgraded.db.prepare('SELECT name FROM schema_migrations ORDER BY name').all().map(row=>row.name),['001_core.sql','002_operations.sql','003_reports.sql','004_bills.sql','005_assistant_actions.sql','006_conversations.sql','007_outbox_media.sql','008_financial_companion.sql','009_companion_proposals.sql','010_transaction_monitor.sql','011_base_routing.sql']);assert.ok(upgraded.db.prepare("SELECT name FROM sqlite_master WHERE name='bill_proposals'").get());assert.ok(upgraded.db.prepare("SELECT name FROM sqlite_master WHERE name='financial_goals'").get());assert.ok(upgraded.db.prepare("SELECT name FROM sqlite_master WHERE name='companion_proposals'").get());assert.ok(upgraded.db.prepare("SELECT name FROM sqlite_master WHERE name='transaction_observations'").get());assert.ok(upgraded.db.prepare("SELECT name FROM sqlite_master WHERE name='telegram_route_ledger'").get());assert.equal(upgraded.db.prepare("SELECT origin FROM operations WHERE id='old-op'").get().origin,'telegram_confirmation');assert.deepEqual(upgraded.getPreference('finance_scope'),{includeClosed:true,includeOffBudget:false});assert.equal(upgraded.db.prepare("SELECT value FROM metadata WHERE key='telegram_next_send_at'").get().value,'123456');}finally{upgraded.close();}
});
